...
{
    public class BombermanEventsHandler : NetworkEventsListner
    {
        ...
        {
            var player = sandbox.NetworkInstantiate(_playerPrefab, SpawnPositions[client.Id + 1], Quaternion.identity, client).GetComponent<BombermanController>();

            client.PlayerObject = player.gameObject;
            player.PlayerNumber = client.Id + 1;
        ...
        {
            _freePositions.Enqueue(client.PlayerObject.GetComponent<BombermanController>().SpawnPos);
        }
        ...
        public void RestartGame()
        {
            ...
            foreach (var player in Sandbox.ConnectedPlayers)
                player.PlayerObject.GetComponent<BombermanController>().Respawn();
        }
